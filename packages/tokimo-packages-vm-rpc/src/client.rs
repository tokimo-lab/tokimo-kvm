...
pub struct Client {
    io: Mutex<(
        ReadHalf<Box<dyn Transport>>,
        WriteHalf<Box<dyn Transport>>,
    )>,
}
...
impl Client {
    ...
    pub async fn connect_port(port_name: &str) -> anyhow::Result<Self> {
        ...
        let path = loop {
            if let Some(p) = resolve_virtio_port(port_name)? { break p; }
    ...
    pub async fn hello(&self) -> anyhow::Result<()> {
        match self.call(Request::Hello { protocol_version: PROTOCOL_VERSION }).await? {
            Response::Hello { protocol_version } if protocol_version == PROTOCOL_VERSION => Ok(()),
            Response::Hello { protocol_version } => anyhow::bail!("protocol mismatch: server {protocol_version}"),
            _ => anyhow::bail!("unexpected response"),
        }
    }

    fn io_err<E: std::fmt::Display>(e: E) -> VfsError { VfsError::Io(e.to_string()) }

    pub async fn stat(&self, path: &Path) -> VfsResult<FileAttr> {
        match self.call(Request::Stat { path: path.to_path_buf() }).await.map_err(Self::io_err)? {
            Response::Stat(r) => r, _ => Err(VfsError::Io("bad resp".into())),
        }
    }
    pub async fn list(&self, path: &Path) -> VfsResult<Vec<DirEntry>> {
        match self.call(Request::List { path: path.to_path_buf() }).await.map_err(Self::io_err)? {
            Response::List(r) => r, _ => Err(VfsError::Io("bad resp".into())),
        }
    }
    pub async fn read(&self, path: &Path, offset: u64, len: u32) -> VfsResult<Vec<u8>> {
        match self.call(Request::Read { path: path.to_path_buf(), offset, len }).await.map_err(Self::io_err)? {
            Response::Read(r) => r, _ => Err(VfsError::Io("bad resp".into())),
        }
    }
    pub async fn write(&self, path: &Path, offset: u64, data: &[u8]) -> VfsResult<u32> {
        match self.call(Request::Write { path: path.to_path_buf(), offset, data: data.to_vec() }).await.map_err(Self::io_err)? {
            Response::Write(r) => r, _ => Err(VfsError::Io("bad resp".into())),
        }
    }
    pub async fn create(&self, path: &Path, mode: u32) -> VfsResult<FileAttr> {
        match self.call(Request::Create { path: path.to_path_buf(), mode }).await.map_err(Self::io_err)? {
            Response::Create(r) => r, _ => Err(VfsError::Io("bad resp".into())),
        }
    }
    pub async fn mkdir(&self, path: &Path, mode: u32) -> VfsResult<FileAttr> {
        match self.call(Request::Mkdir { path: path.to_path_buf(), mode }).await.map_err(Self::io_err)? {
            Response::Mkdir(r) => r, _ => Err(VfsError::Io("bad resp".into())),
        }
    }
    pub async fn remove(&self, path: &Path) -> VfsResult<()> {
        match self.call(Request::Remove { path: path.to_path_buf() }).await.map_err(Self::io_err)? {
            Response::Remove(r) => r, _ => Err(VfsError::Io("bad resp".into())),
        }
    }
    pub async fn rmdir(&self, path: &Path) -> VfsResult<()> {
        match self.call(Request::Rmdir { path: path.to_path_buf() }).await.map_err(Self::io_err)? {
            Response::Rmdir(r) => r, _ => Err(VfsError::Io("bad resp".into())),
        }
    }
    pub async fn rename(&self, from: &Path, to: &Path) -> VfsResult<()> {
        match self.call(Request::Rename { from: from.to_path_buf(), to: to.to_path_buf() }).await.map_err(Self::io_err)? {
            Response::Rename(r) => r, _ => Err(VfsError::Io("bad resp".into())),
        }
    }
    pub async fn truncate(&self, path: &Path, size: u64) -> VfsResult<()> {
        match self.call(Request::Truncate { path: path.to_path_buf(), size }).await.map_err(Self::io_err)? {
            Response::Truncate(r) => r, _ => Err(VfsError::Io("bad resp".into())),
        }
    }
}
...
fn resolve_virtio_port(port_name: &str) -> anyhow::Result<Option<std::path::PathBuf>> {
    ...
    for entry in std::fs::read_dir(root)? {
        ...
        if name == port_name {
            let dev = std::path::PathBuf::from(format!("/dev/{}", entry.file_name().to_string_lossy()));
            if dev.exists() { return Ok(Some(dev)); }
        }
    }
    Ok(None)
}
