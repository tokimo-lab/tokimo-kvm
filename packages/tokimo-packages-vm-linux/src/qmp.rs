...
impl QmpClient {
    pub async fn connect(path: &Path) -> anyhow::Result<Self> {
        ...
        let mut me = Self { reader: BufReader::new(r), writer: w };
    ...
    async fn read_message(&mut self) -> anyhow::Result<Value> {
        ...
        if n == 0 { anyhow::bail!("QMP closed"); }
        Ok(serde_json::from_str(&line)?)
    }

    pub async fn execute(&mut self, cmd: &str, args: Option<Value>) -> anyhow::Result<Value> {
        let mut req = serde_json::json!({ "execute": cmd });
        if let Some(a) = args { req["arguments"] = a; }
        let line = format!("{}\n", req);
        self.writer.write_all(line.as_bytes()).await?;
        self.writer.flush().await?;
        loop {
            let msg = self.read_message().await?;
            if let Some(r) = msg.get("return") { return Ok(r.clone()); }
            if let Some(e) = msg.get("error") { anyhow::bail!("QMP error: {}", e); }
        }
    }
    ...
    pub async fn quit(&mut self) -> anyhow::Result<()> {
        let _ = self.execute("quit", None).await; Ok(())
    }
}
